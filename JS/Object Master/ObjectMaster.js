...
const idDivBy3 = pkmnIds.filter(p => p%3 ===0);
...
const onlyTypeIsPoison = pokémon.filter(p => p.types.length ===1 && p.types[0]=="poison").map(p => p.name);
console.log(onlyTypeIsPoison);

// 7 - an array containing just the first type of all the pokémon whose second type is "flying"
const pokWhichSecondTypeFlying = pokémon.filter(p => p.types[1]==="flying").map(p=> p.types[0]);
console.log(pokWhichSecondTypeFlying);

// 8 -a count of the number of pokémon that are "normal" type
const countOfPokNormalType = pokémon.filter(p=> p.types.includes("normal"))
console.log(countOfPokNormalType.length);